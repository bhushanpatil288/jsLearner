...
const Modal = ({id, image, name, title, description, codeSnippet}) => {
  ...
  return (
    <div>
      <button 
        onClick={()=>(
          setTimeout(()=> setRead(true), 5000)
        )}
        command="show-modal" 
        commandfor={id} 
        className={`rounded-md bg-gray-950/5 py-1.5 text-sm font-semibold text-gray-900 hover:bg-gray-950/10 px-5 md:px-2.5 w-full shadow-md flex md:justify-center ${read ? 'bg-green-500 text-white' : ''}`}>
      {name}
      </button>
      <el-dialog>
            ...
            <el-dialog-panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all data-closed:translate-y-4 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in sm:my-8 sm:w-full sm:max-w-lg data-closed:sm:translate-y-0 data-closed:sm:scale-95">
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  <div className="mx-auto flex size-12 shrink-0 items-center justify-center rounded-full bg-red-100 sm:mx-0 sm:size-10 overflow-hidden">
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div className="mt-2 w-100">
                      {description.map((e, idx)=>(
                        <div key={idx}>
                          <p className="text-sm text-gray-500">{e}</p><br/>
                        </div>
                      ))}
                      {codeSnippet != '' ? (
                        <>
              ...
              <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                <button type="button" command="close" commandfor={id} className="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-red-500 sm:ml-3 sm:w-auto">Close</button>
                {/* <button type="button" command="close" commandfor={id} className="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-xs inset-ring inset-ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">Cancel</button> */}
              </div>
            </el-dialog-panel>
          </div>
        </dialog>
